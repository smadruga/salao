                    ...
                    <div class="row">
                        <div class="col-md-3 form-inline">
                            ...
                            <div class="form-group">
                                <div class="btn-group" data-toggle="buttons">
                                    <?php
                                    foreach ($select['Paciente'] as $key => $row) {
                                        (!$query['Paciente']) ? $query['Paciente'] = 'R' : FALSE;

                                        ($key == 'D') ? $show_hide = 'show' : $show_hide = 'hide';
                                        
                                        if ($query['Paciente'] == $key) {                                            
                                            echo ''
                                            . '<label class="btn btn-warning active" name="radio_Paciente" id="radiogeral' . $key . '">'
                                            . '<input type="radio" name="Paciente" id="radiogeral' . $show_hide . '" '
                                                . 'autocomplete="off" value="' . $key . '" checked>' . $row . $show_hide
                                            . '</label>'
                                            ;
                                        } else {
                                            echo ''
                                            . '<label class="btn btn-default" name="radio_Paciente" id="radiogeral' . $key . '">'
                                            . '<input type="radio" name="Paciente" id="radiogeral' . $show_hide . '" '
                                                . 'autocomplete="off" value="' . $key . '" >' . $row . $show_hide
                                            . '</label>'
                                            ;
                                        }
                                    }
                        ...
                        <div class="col-md-4">
                            <label for="idApp_Dentista">Dentista: *</label>
                            <a class="btn btn-xs btn-info" href="<?php echo base_url() ?>tabelas/cadastrar/dentista" role="button"> 
                                <span class="glyphicon glyphicon-plus"></span> <b>Novo Dentista</b>
                            </a>
                            <select data-placeholder="Selecione uma opção..." class="form-control" <?php echo $readonly; ?>
                                    id="idApp_Dentista" name="idApp_Dentista">
                                <option value="">-- Selecione uma opção --</option>
                                <?php
                                foreach ($select['Dentista'] as $key => $row) {
                                    if ($query['idApp_Dentista'] == $key) {
                                        echo '<option value="' . $key . '" selected="selected">' . $row . '</option>';
                                    } else {
                                        echo '<option value="' . $key . '">' . $row . '</option>';
                                    }
